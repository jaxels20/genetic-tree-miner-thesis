...
def main():
    ...
    eventlogs = [f for f in os.listdir(INPUT_DIR) if f.endswith('.xes')]
    ...
    for e in eventlogs:
        ...
        for p in filtering_percentages:
            ...
            pn = Discovery.run_discovery("Inductive Miner", filtered_eventlog)
            evaluator = SingleEvaluator(pn, eventlog)
            fitness = evaluator.get_replay_fitness()['log_fitness']
            results_top_n_filtering[eventlog_name][p] = round(full_log_fitness - fitness, 4)

            # Random filtering
            filtered_eventlog = Filtering.filter_eventlog_random(eventlog, p)
            pn = Discovery.run_discovery("Inductive Miner", filtered_eventlog)
            evaluator = SingleEvaluator(pn, eventlog)
            fitness = evaluator.get_replay_fitness()['log_fitness']
            results_random_filtering[eventlog_name][p] = round(full_log_fitness - fitness, 4)
